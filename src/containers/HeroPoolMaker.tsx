...
const HeroPoolMaker: React.FC<HeroPoolMakerProps> = ({ heroIds, heroes, heroMatchups }) => {
  ...
  const tableRecords = heroIds.map(heroId => ({
    key: heroId,
    ...heroes[heroId],
    matchups: heroMatchups[heroId],
  }));

  const disadvantageTitle = useResponsive({ short: 0, long: 1280 });

  const columns = ['summary', ...heroPoolIds].reduce<ColumnsType<typeof tableRecords[0]>>((accumulator, heroId) => {
    ...
    return [
      ...
      {
        key: heroId,
        title: <span><HeroMatchupAvatar src={heroes[heroId]?.imageUrl} />{heroName}</span>,
        children: [
          {
            ...
            render: (disadvantage: number) => disadvantage ? `${disadvantage.toFixed(2)}%` : '',
            sorter: (a, b) => a.matchups[heroId]?.disadvantage - b.matchups[heroId]?.disadvantage,
          },
          {
          ...
          },
          {
            ...
            title: 'Win Rate',
            align: 'right',
            render: (winRate: number) => winRate ? `${winRate.toFixed(2)}%` : '',
          },
        ]
      }
    ];
  ...
  return (
    <div>
      ...
      <InstructionsList>
        ...
        {heroMatchups && Object.keys(heroMatchups).length > 0 && <AppTable
          pagination={false}
